//! Gröbner bases and finite quotient algebras over prime fields and the rationals.
//!
...
//! `Ideal<Rationals>::check_basis_equality` supplies a separate exact
//! library check under a budget. It has no independent certificate.
//!
//! [`GroebnerBasis::finite_quotient`] constructs a [`FiniteQuotient`] when
//! the standard monomials form a finite set. It computes residue coordinates,
//! multiplication matrices, and characteristic and minimal polynomials.
//! These operations concern the ideal generated by the source basis and
//! have no independent certificate contract. [`ResultEnvelope`] saves the
//! input, basis, and provenance claim; loading it does not verify the claim.
//!
//! ```
...
pub use compute::{
    Backend, Budget, CancellationToken, ComputeError, ComputeOptions, ComputeReport, F4Counters,
    RationalOptions, RationalStop,
};
pub use hilbert::{HilbertError, HilbertSeries};
pub use ideal::{GroebnerBasis, Ideal};
pub use normal_form::{BasisError, DivisionResult, NormalFormError};
pub use poly::Polynomial;
pub use quotient::{FiniteQuotient, MultiplicationMatrix, QuotientError, UnivariatePolynomial};
pub use rational_check::{EqualityCheckError, RationalEqualityCheck, RationalEqualityMetrics};
pub use result::{ClaimedProvenance, EnvelopeDomain, EnvelopeError, ResultEnvelope};
pub use ring::{
    Coefficient, Domain, DomainOps, Established, ExpressionError, Felt, ModularLift, ParseError,
    PolynomialRing, PrimeField, PrimeOps, RationalMeta, RationalOps, Rationals, RingError,
};
